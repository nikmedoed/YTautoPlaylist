// Content runtime message listener. Handles playlist state broadcasts, collect progress, playback commands, and UI synchronization.
import {
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === "playlist:stateUpdated") {
    ...
    const currentQueue = Array.isArray(playlistState?.currentQueue?.queue)
    ...
    if (current && playlistState.currentVideoId === current) {
      ...
      current &&
      !currentQueue.some((item) => item && item.id === current)
    ) {
