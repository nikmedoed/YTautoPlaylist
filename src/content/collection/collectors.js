// Content collection helpers. Extracts video ids from current, visible, or full YouTube page scopes.
import {
  delay,
  getCurrentVideoId,
  PAGE_COLLECTION_LIMIT,
  PAGE_SCROLL_DELAY,
  PAGE_SCROLL_IDLE_LIMIT,
  PAGE_SCROLL_MAX_LOOPS,
  VIDEO_CARD_SELECTOR,
} from "../core/base.js";
import { findVideoIdInCard } from "../video-cards/targets.js";

export function collectVideoIds(scope = "visible") {
  if (scope === "current") {
...
export function collectVisibleVideoIds({ includeCurrent = true } = {}) {
  const ids = new Set();
...
// Scrolls/listens for YouTube continuation batches until the page stops yielding new video ids or a caller cancels.
export async function collectPageVideosWithContinuation(options = {}) {
